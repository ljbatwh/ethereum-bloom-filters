import {
  ...
  isAddress,
  isBloom, isBloomAllSetInPositions,
...
const blockBloom = ''
const transactionBloom = '';
const tokenAddress = '0x6eebe75caf9c579b3fba9030760b84050283b50a';
const transferTopic = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef';
const BUSDTransactionBloom = '';
describe('isAddress', () => {
...
describe('isInBloom', () => {
  it('should return true if value is in bloom passing in hex string', () => {
    ...
    expect(isInBloom(blockBloom, tokenAddress)).toBe(true);
    expect(isInBloom(transactionBloom, tokenAddress)).toBe(true);
    expect(isInBloom(blockBloom, transferTopic)).toBe(true);
    expect(isInBloom(transactionBloom, transferTopic)).toBe(true);
    expect(isInBloom(BUSDTransactionBloom, transferTopic)).toBe(true);
    expect(isInBloom(BUSDTransactionBloom, tokenAddress)).toBe(false);
  });
...
describe('calculateBitPositions', () => {
  it('should return valid bitPositions', () => {
    expect(
        calculateBitPositions('0x58a4884182d9e835597f405e5f258290e46ae7c2'),
    ).toEqual([331, 2002, 727]);
    expect(calculateBitPositions(tokenAddress)).toEqual([1288, 772, 1312]);
    expect(calculateBitPositions(transferTopic)).toEqual([1443, 481, 1060]);
  });
});

describe('isBloomAllSetInPositions', () => {
  it('bloomFilter should return true', () => {
    expect(isBloomAllSetInPositions(bloomFilter, [331, 2002, 727]));
  });

  it('blockBloom should set for tokenAddress and transferTopic', () => {
    expect(isBloomAllSetInPositions(blockBloom, [1288, 772, 1312, 1443, 481, 1060])).toBe(true);
    expect(isBloomAllSetInPositions(transactionBloom, [1288, 772, 1312, 1443, 481, 1060])).toBe(true);
    expect(isBloomAllSetInPositions(BUSDTransactionBloom, [1443, 481, 1060])).toBe(true);
    expect(isBloomAllSetInPositions(BUSDTransactionBloom, [1288, 772, 1312])).toBe(false);
    expect(isBloomAllSetInPositions(BUSDTransactionBloom, [1288, 772, 1312, 1443, 481, 1060])).toBe(false);
  });
});
